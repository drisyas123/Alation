def run_script(fileName,sheetName, skipRows, domain_name,businessGlossaryID, data_steward):
   try:
      # Import dependencies
      import os
      ...
      # URLs
      refreshTokenURL = BASEURL + "integration/v1/createRefreshToken/"
      ...
      # INPUT
      # read excel
      # inputs for file_name = relative path, sheetname, skiprows
      df_catalog = pd.read_excel(fileName, sheet_name=sheetName, skiprows=int(skipRows))
      # print(df_catalog.head(3))

      # remove first column
      # df_catalog = df_catalog.iloc[:,1:]
      ...
      df_catalog_1 = df_catalog_1.applymap(lambda x: x.strip() if isinstance(x, str) else x)

      # INPUT
      # filter for only the rows with Data Domain\n [Level 1] as 'Fund and Investor Accounting' just for safekeeping
      df_catalog_1 = df_catalog_1[df_catalog_1['[Internal Purposes only] Assignment'] == domain_name]

      # # print(df_catalog_1.head(3))

      # filter for CDEs
      df_catalog_1 = df_catalog_1[df_catalog_1['Preliminary Assignment of Critical Data Element [CDE]'] == 'CDE']

      # # print(df_catalog_1.shape)
      ...
      # # Delete all domains from Alation
      # deleteDomains = requests.delete(domainUrl, json=domainIdDict, headers=tokenHeader, verify=False)
      # print(deleteDomains.text)
      ...
      # Search for Domain iD instead of domain name
      ...
      searched_domain = search_result['results'][0]['title']
      
      #if not response.ok:
      if (search_result['total']) == 1:
         print('Domain already exists: '+ searched_domain)
      else:
         ...
         # # print(subDomainUploadResponse.text)
         ...
         # # print(df_subdomainUploadResponse.head(10))
         ...
         # print(dataClassUploadResponse.text)

         # convert domainUploadResponse to dataframe
         df_dataClassUploadResponse = pd.DataFrame(dataClassUploadResponseDict)

         # print(df_dataClassUploadResponse.head(50))
         ...
         if data_steward == "Y":
            ...
            # print(df_catalog_2.head(10))

         #END OF 'DO NOT RUN IF DATA STEWARDS ARE NOT BEING UPLOADED'

         # Add Code to check if glossary is already created, if not create one for the domain - Nomenclaure as  '<Domain Name> Business Glossary'ArithmeticError

         # INPUT
         # Add the glossary ids for each Business Term

         # businessGlossaryID = 4
         ...
         #INPUT
         termsUrlwithParams = f"{termsUrl}?glossary_id={businessGlossaryID}&limit={fetchLimit}"
         ...
         # print(df_catalog_3_terms.head(10))
      ...
      # Add another output saying 'Upload Completed'
...
if __name__ == "__main__":
    import sys
    try:
        fileName = input("Please enter the path of the file: ")
        sheetName = input("Please enter the sheet Name of the file used: ")
        skipRows = input("Please enter the number of rows to be skipped: ")
        domain_name = input("Please enter the Domain name to be filtered: ")
        businessGlossaryID = input("Please enter the Glossary ID from Alation: ")
        data_steward = input("Do you want to update Data Steward?(Y/N): ")
        data_steward = data_steward.upper()
        if not businessGlossaryID:
            print("Glossary ID is required to proceed.")
            sys.exit(1)
        
        
        # Run the script with the provided inputs
        run_script(fileName,sheetName,skipRows, businessGlossaryID, domain_name, data_steward)
